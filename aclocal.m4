dnl
dnl Copyright (C) 2000-2008 QoSient, LLC.
dnl
      ...
      if test ! -z "$READLINELIB"; then
         ...
         if test ! -z "$READLINEHOOK"; then
            ...
            if test ! -z "$READLINEREPLACELINE"; then
               $1="$READLINELIB"
               cat >> confdefs.h <<\EOF
#define ARGUS_READLINE 1
#define ARGUS_READLINE_EVENT_HOOK 1
EOF
            fi
         fi
      fi
...
ARGUS_MYSQL=""
   ...
   if test "$with_mysql" != no; then
      ...
      for dir in ${with_mysql} /usr /usr/local /usr/local/mysql /usr/mysql ; do
         if test "$ac_cv_found_mysql" != yes; then
            ...
            if test -d $dir/lib/mysql; then
               ac_cv_mysql_where_lib=$dir/lib/mysql
            else
               if test -d $dir/lib; then
                  ac_cv_mysql_where_lib=$dir/lib
               fi
            fi
