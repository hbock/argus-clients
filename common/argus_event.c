...
/*
 * $Id: //depot/argus/clients/common/argus_event.c#9 $
...
ArgusProcessEvent (struct ArgusParserStruct *parser, struct ArgusEventObject *event, struct ArgusRecordStruct *ns)
{
   ...
   if (!(event->target)) {
      ...
      for (i = 1; i < (NTAIS_ETARGETS + 1); i++) {
         if (event->target & (0x01 << (i - 1))) {
            switch ((0x01 << (i - 1))) {
               case NTAIS_DATABASE: retn = ArgusProcessSQLEvent(parser, event, ns); break;
               case NTAIS_SYSLOG :  retn = ArgusProcessSyslogEvent(parser, event, ns); break;
               case NTAIS_FILE:     retn = ArgusProcessFileEvent(parser, event, ns); break;
               case NTAIS_TERM:     retn = ArgusProcessTermEvent(parser, event, ns); break;
               default:  break;
            }
...
ArgusProcessSQLEvent (struct ArgusParserStruct *parser, struct ArgusEventObject *event, struct ArgusRecordStruct *ns)
{
   ...
   char sbuf[MAXSTRLEN], mbuf[MAXSTRLEN], username[1024], *uid; 
   char userbuf[MAXSTRLEN], *sptr, *hptr; 
   ...
   if (event->type & (NTAIS_EVENT | NTAIS_CONDITION)) {
      if (ns != NULL) {
         ...
         if (time != NULL) {
            if ((time->src.start.tv_sec  == time->src.start.tv_sec) &&
               ...
               event->type |= NTAIS_EVENT;
            } else {
               event->type |= NTAIS_CONDITION;
            }
         }
      }
   }
   ...
   while ((aname = strtok (accounts, " ")) != NULL) {
      char *name = NULL, *pass = NULL, *stat = NULL;
...
#ifdef ARGUSDEBUG
   {
      ...
      for (i = 0, seen = 0; i < NTAIS_NTYPES; i++) {
   ...
   if (ns == NULL) {
      if ((stvp.tv_sec = parser->startime_t) == 0x7FFFFFFF)
         gettimeofday(&stvp, 0L);

      if ((etvp.tv_sec  = parser->lasttime_t) == 0)
...
ArgusProcessSyslogEvent (struct ArgusParserStruct *parser, struct ArgusEventObject *event, struct ArgusRecordStruct *ns)
{
   ...
   for (i = 0; i < NTAIS_NFACILITY; i++)
      if (RaSQLEventFacilities[i].v == event->facility)
         facility = RaSQLEventFacilities[i].s;

   sprintf (sbuf, "[ntais] %s:%s:'%s' ", facility, event->metadata, event->message);
...
ArgusProcessFileEvent (struct ArgusParserStruct *parser, struct ArgusEventObject *event, struct ArgusRecordStruct *ns)
{
   ...
   sprintf (&sbuf[strlen(sbuf)], "[ntais] %s:%s:'%s' ", facility, event->metadata, event->message);
