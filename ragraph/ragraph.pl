...
sub RagraphProcessArgusData {
   my @args = "";
   if ($histo) {
      @args = ("$RABINS -M hard zero -p6 -GL0 -s ", @arglist, "> $tmpfile");
   } else {
      @args = ("$RABINS -M hard zero -p6 -GL0 -s ltime ", @arglist, "> $tmpfile");
   }
...
sub RagraphGenerateRRDParameters {
   ...
   if (@typeobjects + 0) {
      ...
      for my $objs (sort { $objectlist{$b} <=> $objectlist{$a} } keys %objectlist) {
         if ($objs && (!(($objs eq "0.0.0.0") || ($objs eq "0:0:0:0:0:0") || ($objs eq " ")))) {
      ...
      for (my $i = 0; $i < (@objects + 0); $i++) {
         for (my $x = 0; $x < (@dataobjects + 0); $x++) {
            for ($columns[$dataobjects[$x]]) {
               /Tot|Host|Src|Out|SApp|ds/             and do {
   ...
   if ($protoindex >= 0) {
      ...
      if ($serviceindex >= 0) {
         ...
         microsoftds => "#808080",        msnp => "#808080", msstreaming => "#808080",
         ...
         microsoftds => "#b0b0b0",        msnp => "#b0b0b0", msstreaming => "#b0b0b0",
   ...
   if (@typeobjects + 0) {
      ...
      for (my $i = 0; $i < (@objects + 0); $i++) {
         if (!($objects[$i] eq "")) {
            ...
            for (my $x = 0; $x < (@dataobjects + 0); $x++) {
               for ($columns[$dataobjects[$x]]) {
                  /Host|Tot|Src|Out|SApp|ds/   and do {
