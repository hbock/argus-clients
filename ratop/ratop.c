...
/* 
 * $Id: //depot/argus/argus-3.0/clients/ratop/ratop.c#73 $
 * $DateTime: 2006/05/24 09:49:59 $
 * $Change: 856 $
 */
...
ArgusClientInit (struct ArgusParserStruct *parser)
{
   ...
   int i = 0, size = 1;
#if defined(ARGUS_CURSES) && defined(ARGUS_READLINE)
   int keytimeout;
            ...
            for (x = 0, z = 0, i = RaWindowStartLine; x < cnt; x++, i++) {
               if ((ns = (struct ArgusRecordStruct *) queue->array[i]) != NULL) {
                  ...
                  *(unsigned int *)buf = 0;
                  ...
                  mvwaddnstr (window, x + 1, 0, tbuf, RaScreenColumns);
