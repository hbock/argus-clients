 ...
 *
 * 
 * $Id: //depot/argus/clients/clients/ranonymize.c#17 $
...
#define RANON_DSR_TYPES			19

#define RANON_DSR_TRANS			0
#define RANON_DSR_FLOW			1
#define RANON_DSR_TIME			2
#define RANON_DSR_METRIC		3
#define RANON_DSR_AGR			4
#define RANON_DSR_NET			5
#define RANON_DSR_VLAN			6
#define RANON_DSR_MPLS			7
#define RANON_DSR_JITTER		8
#define RANON_DSR_IPATTR		9
#define RANON_DSR_PSIZE			10
#define RANON_DSR_SUSER			11
#define RANON_DSR_DUSER			12
#define RANON_DSR_MAC			13
#define RANON_DSR_ICMP			14
#define RANON_DSR_ENCAPS		15
#define RANON_DSR_TADJ			16
#define RANON_DSR_COR			17
#define RANON_DSR_COCODE		18
...
ArgusClientInit (struct ArgusParserStruct *parser)
{
   ...
   if (!(parser->RaInitialized)) {
      ...
      if ((mode = parser->ArgusModeList) != NULL)
         ...
         ArgusDSRFields[RANON_DSR_FLOW] = 1;
         ArgusDSRFields[RANON_DSR_NET] = 1;
         ArgusDSRFields[RANON_DSR_ICMP] = 1;
      }
...
usage ()
{
   ...
   fprintf (stderr, "Ranonymize Version %s\n", version);
   fprintf (stderr, "usage: %s [-M [modes] [+|-]dsr [dsr ...]] [ra-options]\n\n", ArgusParser->ArgusProgramName);

   fprintf (stderr, "options: -M replace      nonymize dsrs and overwrite current file.\n");
...
{
   struct ArgusInput *ArgusInput = argus->input;
   ...
   switch (argus->hdr.type & 0xF0) {
      ...
      case ARGUS_FAR: {
         for (x = 0; x < ARGUSMAXDSRTYPE; x++) {
            if (ArgusDSRFields[x]) {
               switch (x) {
                  case RANON_DSR_TRANS:   ArgusAnonymizeTransport(parser, argus); break;
                  case RANON_DSR_TIME:    ArgusAnonymizeTime(parser, argus); break;
                  case RANON_DSR_TADJ:    ArgusAnonymizeTimeAdjust(parser, argus); break;
                  case RANON_DSR_FLOW:    ArgusAnonymizeFlow(parser, argus); break;
                  case RANON_DSR_METRIC:  ArgusAnonymizeMetric(parser, argus); break;
                  case RANON_DSR_AGR:     ArgusAnonymizeAgr(parser, argus); break;
                  case RANON_DSR_NET:     ArgusAnonymizeNetwork(parser, argus); break;
                  case RANON_DSR_VLAN:    ArgusAnonymizeVlan(parser, argus); break;
                  case RANON_DSR_MPLS:    ArgusAnonymizeMpls(parser, argus); break;
                  case RANON_DSR_MAC:     ArgusAnonymizeMac(parser, argus); break;
                  case RANON_DSR_JITTER:  ArgusAnonymizeJitter(parser, argus); break;
                  case RANON_DSR_IPATTR:  ArgusAnonymizeIPattribute(parser, argus); break;
                  case RANON_DSR_SUSER:   ArgusAnonymizeSrcUserData(parser, argus); break;
                  case RANON_DSR_DUSER:   ArgusAnonymizeDstUserData(parser, argus); break;
                  case RANON_DSR_ICMP:    ArgusAnonymizeIcmp(parser, argus); break;
                  case RANON_DSR_COR:     ArgusAnonymizeCorrelate(parser, argus); break;
                  case RANON_DSR_COCODE:  ArgusAnonymizeCountryCode(parser, argus); break;
               }
            }
         }
