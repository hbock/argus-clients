 ...
 * QoSient, LLC
 * 
 * $Id: //depot/argus/clients/clients/rahisto.c#30 $
      ...
      if (parser->Hstr) {
         ...
         switch (retn) {
            case ARGUS_HISTO_RANGE_UNSPECIFIED: {
               ...
               parser->RaHistoStart =  HUGE;
               parser->RaHistoEnd   = -HUGE;
               break;
            }
            ...
            if (ns != NULL) {
               ...
               if ((tagr = (void *)ns->dsrs[ARGUS_AGR_INDEX]) != NULL) {
                  if (parser->ArgusWfileList == NULL) {
                     int len, tlen, numModes, pflag = parser->pflag;
