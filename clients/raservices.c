 ...
 *
 * 
 * $Id: //depot/argus/clients/clients/raservices.c#8 $
      ...
      case ARGUS_FAR: {
         if (flow) {
            if (argus->dsrs[ARGUS_SRCUSERDATA_INDEX] || argus->dsrs[ARGUS_DSTUSERDATA_INDEX]) {
               ...
               if (process) {
                  struct ArgusLabelStruct *label = NULL;
                  int length = 0;
...
*/
                  ...
                  if (found) {
                     if ((label = (struct ArgusLabelStruct *) argus->dsrs[ARGUS_LABEL_INDEX]) == NULL) {
                        int len = strlen(name) + 4;

                        if ((label = ArgusCalloc(1, sizeof(*label))) == NULL)
                           ArgusLog (LOG_ERR, "RaProcessRecord: ArgusCalloc error %s", strerror(errno));

                        if ((label->l_un.svc = calloc(1, len)) == NULL)
                           ArgusLog (LOG_ERR, "RaProcessRecord: ArgusCalloc error %s", strerror(errno));

                        label->hdr.type             = ARGUS_LABEL_DSR;
                        label->hdr.subtype          = ARGUS_SVC_LABEL;
                        label->hdr.argus_dsrvl8.len = 1 + ((len + 3)/4);
                        bcopy (name, label->l_un.svc, strlen(name));

                        argus->dsrs[ARGUS_LABEL_INDEX] = (struct ArgusDSRHeader *)label;
                        argus->dsrindex |= (0x01 << ARGUS_LABEL_INDEX);
                        label->hdr.argus_dsrvl8.len = 1 + (len/4);
                     }

                  } else
                     argus->dsrs[ARGUS_LABEL_INDEX] = NULL;
               }
            }
